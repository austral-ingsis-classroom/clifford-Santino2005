...
public class ls implements Command {
  ...
  private Result Conditions(String[] split, FileManager dirState) {
    if (split[0].equals("--ord")) {
      if (split[1].equals("asc")) {
        return new Result(
            dirState,
            formatListContent(getSortedContent(dirState.getCurrent().listContent(), true)));
      } else if (split[1].equals("desc")) {
        return new Result(
            dirState,
            formatListContent(getSortedContent(dirState.getCurrent().listContent(), false)));
      } else {
  ...
  private String formatListContent(List<FileSystem> content) {
    ...
    return content.stream().map(FileSystem::getName).collect(Collectors.joining(" "));
  }
}
